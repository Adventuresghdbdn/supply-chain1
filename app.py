...
def load_model():
    try:
        model_path = os.path.join(BASE_DIR, "forecast_model_final.pkl")
        feature_path = os.path.join(BASE_DIR, "feature_columns_final.pkl")
...
def forecast_next_days(model, df_current, days):
    ...
    for _ in range(days):
        ...
        missing_cols = [col for col in feature_columns if col not in future_row.columns]
...
if st.button("Generate Forecast"):
    ...
    ax.set_title(f"{days}-Day Demand Forecast")
